...
export const adapterCreator: worker.AdapterCreator<BackendRow> = (context: data.RowsStoreContext) => {
  ...
  const processUpdateRow = (row: BackendRow): BackendRow[] => {
    ...
    if (value !== undefined) {
      ...
      if (oldGroupId && oldGroupId !== newGroupId) {
        ...
        const oldParent = groups.get(oldGroupId);
        if (oldParent) results.push({ ...oldParent });
      }
  ...
  return {
    ...
    getRowDeltaMessage: (json: any) => ({
      ...json,
      add: json?.add?.flatMap(processAddRow),
      update: json?.update?.flatMap(processUpdateRow),
      remove: json?.remove?.flatMap(processRemoveRow),
    }),
  };
};